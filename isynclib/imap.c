/* $Id: imap.c,v 1.4 2009/01/22 22:23:03 rcgreenw Exp $
 *
...
imap_connect (config_t * cfg)
{
  ...
  int use_ssl;
...
#endif
	imap->error = 1;
	strcpy(temp_error, "Error Resolving Host ");
	/* temp_error = strdup("Error Resolving Host "); */
	strcat(temp_error, cfg->host);
	imap->error_message = malloc(strlen(temp_error) + 1);
	strcpy(imap->error_message, temp_error);
	/* imap->error_message = strdup(temp_error); */
...
#endif
	imap->error = 1;
	strcpy(temp_error, "Error Connecting to ");
	/* temp_error = strdup("Error Connecting to "); */
	strcat(temp_error, inet_ntoa(addr.sin_addr));
	/* imap->error_message = strdup(temp_error); */
	imap->error_message = malloc(strlen(temp_error) + 1);
	strcpy(imap->error_message, temp_error);
	goto bail;
	...
	{
		imap->error = 1;
		imap->error_message = strdup("Error start_tls");
      		goto bail;
	}
	use_ssl = 1;
      }
...
#endif
	imap->error = 1;
	imap->error_message = strdup("IMAP Error: no greeting response");
	goto bail;
      }
      arg = next_arg (&rsp);
...
#endif
	imap->error = 1;
	imap->error_message = strdup("IMAP Error: invalid greeting response");
...
#endif
	{
	  ...
	  if (imap_exec (imap, "LOGIN \"%s\" \"%s\"", cfg->user, cfg->pass))
	  {
		...
		/* temp_error = strdup("IMAP Error: LOGIN Failed for user "); */
		strcpy(temp_error, "IMAP Error: LOGIN Failed for user ");
		strcat(temp_error, cfg->user);
		/* imap->error_message = strdup(temp_error); */
		imap->error_message = malloc(strlen(temp_error) + 1 );
		strcpy(imap->error_message, temp_error);
